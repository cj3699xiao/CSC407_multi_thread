...
void sigTurnHandler(int sig, siginfo_t* infoPtr, void* dataPtr){

  int joey_index = getJoeyIndex(infoPtr->si_pid);
  pid_t Cur = infoPtr->si_pid;



  if((NUM_TURNS_MUST_SUCCESSFULLY_MAKE>joeysNumSuccessfulTurns[joey_index])){
    if(wasJoeysTurnCorrect(joey_index,sig)){
      printf("Mall: Joey # %d turned correctly!  (%s.)\n",joey_index, (sig==RIGHT_TURN_SIGNAL)?"right":"left");
      fflush(stdout);
      printf("Mall: Joey # %d must make %d  more correct turns. \n",joey_index,NUM_TURNS_MUST_SUCCESSFULLY_MAKE - joeysNumSuccessfulTurns[joey_index]);
      fflush(stdout);
      kill(Cur,CORRECT_TURN_SIGNAL);
      joeysNumSuccessfulTurns[joey_index]++;


    }else{
      printf("Mall: Joey # %d  made the wrong turn!  (%s.)\n",joey_index,(sig==RIGHT_TURN_SIGNAL)?"right":"left");
      fflush(stdout);
      kill(Cur,WRONG_TURN_SIGNAL);
...
void install_turn_handle(){

  struct sigaction action_turn;
  memset(&action_turn,'\0',sizeof(action_turn));
  sigemptyset(&action_turn.sa_mask);
  action_turn.sa_flags = SA_SIGINFO | SA_RESTART; 
  action_turn.sa_sigaction = sigTurnHandler;
  sigaction (LEFT_TURN_SIGNAL,&action_turn,NULL);
  sigaction (RIGHT_TURN_SIGNAL,&action_turn,NULL); 
} 
