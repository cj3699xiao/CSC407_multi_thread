				...
				)
{
  ...
  // YOUR CODE HERE TO return 0 if the corresponding integers are not in their proper ranges (look at the constant names):
        if(
*(argv[*(bufferLenPtr)])> MAX_BUFFER_LEN | *(argv[*(bufferLenPtr)])< MIN_BUFFER_LEN
|*(argv[*(numMakersPtr)])> MAX_NUM_MAKERS | *(argv[*(numMakersPtr)])< MIN_NUM_MAKERS
|*(argv[*(numOrderersPtr)])> MAX_NUM_ORDERERS | *(argv[*(numOrderersPtr)])< MIN_NUM_ORDERERS 
|*(argv[*(numSandwichesPtr)])> MAX_NUM_SANDWICHES | *(argv[*(numSandwichesPtr)])< MIN_NUM_SANDWICHES	
 ) return (0);
				...
				)
{
  ...
  for  (i = 0;  i < playerPtr->numSandwiches_;  i++)
  {
    ...
    //  Then free() it
    sandwichPtr = orderList.getOrder(i);
    free(orderList.getOrder(i));
				...
				)
{
  ...
  //  Give them all the address of their corresponding ordererArray[] object
	for(int i=0;i<sizeof(makerTidArray);i++){
		pthread_create(&(makerTidArray[i]),NULL,makerFnc,(void*)&(makerArray[i]));
	}
	
	for(int j=0;j<sizeof(ordererTidArray);j++){
 ...
 // pthread_cond_wait(&notFull_,&lock_);
  //pthread_cond_wait(&notEmpty_,&lock_);
	...
	for(int i1=0;i1<sizeof(makerTidArray);i1++){
	pthread_join(makerTidArray[i1],NULL);
	pthread_join(ordererTidArray[i1],NULL);
